...
//session을 이용하여 userId를 찾아서 DB에 저장된 email을 가져온다. ex) codingspecialist@naver.com
	...
	boolean rightCode = 
			SHA256.getEncrypt((String)session.getAttribute("checkEmail"), "cos").equals(code) ? true : false;
	PrintWriter script = response.getWriter();
	if(rightCode == true){
		script.println("<script>");
		script.println("alert('이메일 인증에 성공하였습니다.')");
		script.println("location.href='" + request.getContextPath() + "/account/updateEmail'");
		script.println("</script>");
	} else{
		script.println("<script>");
